...
void wax_printStackAt(lua_State *L, int i) {
    int t = lua_type(L, i);
    wax_log(LOG_DEBUG, @"(%s) ", lua_typename(L, t));
    
    switch (t) {
        case LUA_TSTRING:
            wax_log(LOG_DEBUG, @"'%s'", lua_tostring(L, i));
            break;
        case LUA_TBOOLEAN:
            wax_log(LOG_DEBUG, lua_toboolean(L, i) ? @"true" : @"false");
            break;
        case LUA_TNUMBER:
            wax_log(LOG_DEBUG, @"'%g'", lua_tonumber(L, i));
            break;
            
        default:{
            
            wax_instance_userdata* userdata =(wax_instance_userdata *)lua_touserdata(L,i);
            wax_log(LOG_DEBUG, @"data:%p object:%p name:%@ count:%zd",
                    userdata,userdata->instance,NSStringFromClass([userdata->instance class]),[userdata->instance retainCount]);
            // wax_log(LOG_DEBUG, @"%p", lua_topointer(L, i));
            break;
        }
    }
}
...
void wax_printTable(lua_State *L, int t) {
    ...
    if (t <= 0 || t > lua_gettop(L)) {
        wax_log(LOG_DEBUG, @"%zd is not within stack boundries.\n", t);
        return;
    }
    else if (!lua_istable(L, t)) {
        wax_log(LOG_DEBUG, @"Object at stack index %zd is not a table.\n", t);
        return;
    }
    ...
    int i=0;
    while (lua_next(L, t) != 0) {
        wax_log(LOG_DEBUG,[NSString stringWithFormat:@"  %zd:",++i]);
        wax_printStackAt(L, -1);
...
void wax_log(int flag, NSString *format, ...) {
    if (flag & LOG_FLAGS) {
        ...
        if (flag & LOG_DEBUG) {
            ...
            NSLog(@"[WAX]:%s", [output UTF8String]);
            
        } else if(flag & LOG_FATAL){
...
int wax_fromObjc_64_id(lua_State *L, const char *typeDescription, id buffer1) {
    BEGIN_STACK_MODIFY(L)
    
    id buffer = [buffer1 longValue];
    ...
    switch (typeDescription[0]) {
        ...
        case WAX_TYPE_UNSIGNED_LONG_LONG:
            ...
            break;
        case WAX_TYPE_FLOAT:
            lua_pushnumber(L, [buffer1 floatValue]);
            break;
            
        case WAX_TYPE_DOUBLE:
            lua_pushnumber(L, [buffer1 doubleValue]);
            break;
        ...
        case WAX_TYPE_ID: {
            wax_fromInstance(L, buffer);
            break;
        }
            
        case WAX_TYPE_STRUCT: {
            //            buffer = __va_arg_64(buffer,__va_gen_reg,size);
            //            struct _buffer_16 temp = va_arg(buffer, struct _buffer_16);
...
int wax_fromObjc_64(lua_State *L, const char *typeDescription, void *buffer) {
    BEGIN_STACK_MODIFY(L)
...
int wax_fromObjc_x64(lua_State *L, const char *typeDescription, va_list buffer) {
    BEGIN_STACK_MODIFY(L)
    ...
    switch (typeDescription[0]) {
        ...
        case WAX_TYPE_POINTER:
            lua_pushlightuserdata(L, va_arg(buffer, void **));
            break;
            
        case WAX_TYPE_CHAR: {
            char c = va_arg(buffer, char);
            if (c <= 1) lua_pushboolean(L, c); // If it's 1 or 0, then treat it like a bool
            else lua_pushinteger(L, c);
            break;
        }
        ...
        case WAX_TYPE_SHORT:
            lua_pushinteger(L, va_arg(buffer, short));
            break;
            
        case WAX_TYPE_INT:
            lua_pushnumber(L, va_arg(buffer, int));
            break;
            
        case WAX_TYPE_UNSIGNED_CHAR:
            lua_pushnumber(L, va_arg(buffer, unsigned char));
            break;
            
        case WAX_TYPE_UNSIGNED_INT:
            lua_pushnumber(L, va_arg(buffer, unsigned int));
            break;
        ...
        case WAX_TYPE_LONG:
            lua_pushnumber(L, va_arg(buffer, long));
            break;
            
        case WAX_TYPE_LONG_LONG:
            lua_pushnumber(L,  va_arg(buffer, long long));
            break;
            
        case WAX_TYPE_UNSIGNED_LONG:
            lua_pushnumber(L, va_arg(buffer, unsigned long));
            break;
            
        case WAX_TYPE_UNSIGNED_LONG_LONG:
            lua_pushnumber(L, va_arg(buffer, unsigned long long));
            break;
            
        case WAX_TYPE_FLOAT:
            lua_pushnumber(L, va_arg(buffer, float));
            break;
            
        case WAX_TYPE_DOUBLE:
            lua_pushnumber(L, va_arg(buffer, double));
            break;
            
        case WAX_TYPE_C99_BOOL:
            lua_pushboolean(L, va_arg(buffer, BOOL));
            break;
            
        case WAX_TYPE_STRING:
            lua_pushstring(L, va_arg(buffer, char **));
            break;
            
        case WAX_TYPE_ID: {
            wax_fromInstance(L, va_arg(buffer, id));
            break;
        }
        ...
        case WAX_TYPE_STRUCT: {
            ...
            if (size == 16) {
                //支持CGSize 和 NSRange
                if (strstr(typeDescription, "CGSize") > 0) {
                    const CGSize b_16 = va_arg(buffer, CGSize);
                    wax_fromStruct(L, typeDescription, &b_16);
                }
                else if(strstr(typeDescription, "NSRange") > 0) {
                    const NSRange b_16 = va_arg(buffer, NSRange);
                    wax_fromStruct(L, typeDescription, &b_16);
                }
            } else if (size == 32) {
                const CGRect b_32 = va_arg(buffer, CGRect);
                wax_fromStruct(L, typeDescription, &b_32);
            }
        ...
        case WAX_TYPE_SELECTOR:
            lua_pushstring(L, sel_getName(va_arg(buffer, SEL)) );
            break;
            
        case WAX_TYPE_CLASS: {
            id instance = va_arg(buffer, id);
            wax_instance_create(L, instance, YES);
            break;
        }
...
int wax_fromObjc_NWax(lua_State *L, const char *typeDescription, void *buffer) {
    BEGIN_STACK_MODIFY(L)
            ...
            *outsize = sizeof(id);
            ...
            switch (lua_type(L, stackIndex)) {
                ...
                case LUA_TFUNCTION: {
                    ...
                    /** @Author atterpeng, 15-04-14 11:04:18, 根据栈顺序，压入正确的lua函数对象  */
                    if (stackIndex>0) {
...
void wax_selectorsForName(const char *methodName, SEL possibleSelectors[3]) {
    ...
    if(argCount > 0)
    {
        possibleSelectors[2] = sel_getUid(methodName);
    }
    else
    {
        possibleSelectors[2] = nil;
    }
